...
static int ParseMotion3(const char* jsonString, csmJsonTokenType type, int begin, int end, void* motionParserContext)
{
  ...
  switch (context->State)
  {
    ...
    case ReadingTarget:
    {
      ...
      {
        context->Buffer->Curves[context->CurveIndex].Type = csmModelFloat;
      }
      else if (DoesStringStartWith(jsonString + begin, "Parameter"))
      {
        context->Buffer->Curves[context->CurveIndex].Type = csmParameterFloat;
      }
      else if (DoesStringStartWith(jsonString + begin, "PartOpacity"))
      {
        context->Buffer->Curves[context->CurveIndex].Type = csmPartOpacityFloat;
      }
    ...
    {
      // Hash ID..
      if (context->Buffer->Curves[context->CurveIndex].Type != csmModelFloat)
      {
        context->Buffer->Curves[context->CurveIndex].Id = csmHashSubString(jsonString, begin, end);
      }


      // TODO  ... or assign model float type.
      else
      {
        ;
      }
